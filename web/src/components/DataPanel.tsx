...
function Card({ children, color = 'cyan' }: { children: React.ReactNode; color?: string }): JSX.Element {
  return (
    <div
      style={{
...
export function DataPanel({ snapshot, connected }: Props): JSX.Element {
  if (!snapshot) {
    return (
  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', top: 16, left: 16, zIndex: 5, width: 320 }}>
        <Card>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6 }}>
            ...
            <div style={{ marginLeft: 'auto', fontSize: 11, color: connected ? 'var(--ok)' : 'var(--crit)' }}>
              {connected ? '● live' : '● offline'}
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            <div style={{ fontSize: 32, fontWeight: 700, color: 'var(--neon-cyan)', textShadow: '0 0 12px rgba(90,240,255,0.4)' }}>
          ...
          <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--dim)' }}>
            <div>
              session <span style={{ color: 'var(--neon-lime)' }}>{sessionPct.toFixed(0)}%</span>
            </div>
            <div>
              resets in <span style={{ color: 'var(--neon-pink)' }}>{formatCountdown(claude.resetsAtMs)}</span>{' '}
      ...
      <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 5, width: 240 }}>
        <Card>
          <div style={{ fontSize: 11, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
            host
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 36px', alignItems: 'center', columnGap: 8, rowGap: 6 }}>
            ...
            <div style={{ gridColumn: '2 / span 2', fontSize: 11 }}>
              ...
              {hw.gpuPct !== null ? <span style={{ color: 'var(--dim)' }}> · gpu {hw.gpuPct.toFixed(0)}%</span> : null}
            </div>
          </div>
        </Card>
      </div>
      ...
      <div style={{ position: 'absolute', top: 200, left: 16, zIndex: 5, width: 320 }}>
        <Card>
          <div style={{ fontSize: 11, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
            token flow
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 700 }}>{formatTokens(claude.buckets.input)}</div>
          ...
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--dim)' }}>
            <div>cache write <span style={{ color: 'var(--text)' }}>{formatTokens(claude.buckets.cacheCreation)}</span></div>
            <div>cache read <span style={{ color: 'var(--text)' }}>{formatTokens(claude.buckets.cacheRead)}</span></div>
          </div>
        </Card>
      </div>
            ...
            <AnimatePresence initial={false}>
              {claude.recent.slice(0, 6).map((r) => (
                <motion.div
                  ...
                  style={{ display: 'grid', gridTemplateColumns: '60px 56px 1fr auto', gap: 8, fontSize: 11, padding: '3px 0' }}
                >
      ...
      <div style={{ position: 'absolute', bottom: 16, left: 0, right: 0, display: 'grid', placeItems: 'center', zIndex: 5 }}>
        <div
          style={{
            padding: '6px 14px',
